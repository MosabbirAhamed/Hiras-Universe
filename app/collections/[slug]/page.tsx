      ...
      <div className="site-container pb-16 pt-10 sm:pb-20 sm:pt-14 lg:pb-24 lg:pt-16">
        <header className="mb-10 border-b border-[var(--color-border)] pb-8 sm:mb-12 sm:pb-10">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between sm:gap-8">
            <div className="max-w-3xl">
              <h1 className="font-serif text-4xl font-semibold leading-[1.05] text-[var(--color-heading)] sm:text-5xl lg:text-6xl">{title}</h1>
              <p className="mt-4 max-w-xl text-sm leading-7 text-[var(--color-muted)] sm:text-base">Carefully tailored styles celebrating everyday elegance, comfort, and modest grace.</p>
            </div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--color-muted)]">{displayProducts.length} {displayProducts.length === 1 ? 'piece' : 'pieces'}</p>
          </div>
        </header>
        ...
        {displayProducts.length > 0 ? (
          ...
          <div className="storefront-card flex min-h-[300px] flex-col items-center justify-center px-6 py-16 text-center sm:px-12">
            ...
            <h2 className="font-serif text-2xl font-semibold text-[var(--color-heading)]">The next edit is on its way</h2>
            <p className="mt-3 max-w-md text-sm leading-6 text-[var(--color-muted)]">There are no available pieces in this collection right now.</p>
          </div>
        )}
      </div>
    </main>
